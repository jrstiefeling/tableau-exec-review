...
| **Sales-motion split** (`acv-sources`, replacing `mix-acv`) | ❌ **twice over** | No product-motion dimension exists, and the replacement needs a second absent dimension besides: nothing in either model distinguishes an **expansion** booking from a **new logo**, and nothing describes **Cloud Migration** as a deal shape. The product half was already unavailable. Closest governed grouping is `APM_L120` (family) / `APM_L218` (sub-product: `'Tableau Next'`, `'Tableau Server'`), and `APM_Specialist_Cloud_Routing` on the specialist side | FCST / SPEC | — | remove `APM_L120 = 'Other'` (SDM business preference) |
...
| `acv-sources` *(replaces `mix-acv`)* | `ACV_clc` grouped by a five-value motion taxonomy × fiscal quarter, six quarters | `Sls_Forecasting_Metrics_Expanded` | fiscal quarter × motion | `Org62 Opportunity` → extract → APM hierarchy → **a motion taxonomy that exists only in the deck** | `lineage`'s "Product SKU Taxonomy" → the real name is the **APM product hierarchy (L1/L2/L3)**. The motion grouping does not exist — see §5.4 — and `certifiedBy` names **nobody** for it. The series uses `Close_Date_Fiscal_Year_Datepart_clc`, not `Close_Date_Relative_Year_clc`: relative year is two buckets and a six-quarter series needs six |
...
The portlet in this slot — `acv-sources` now, `mix-acv` before it — has claimed that its split comes from "a governed SKU-to-motion dimension, not from a keyword match on product name." In the real layer there is no motion dimension. What exists is the APM hierarchy, where `'Tableau Next'` and `'Tableau Server'` are both **L2 values** — and FCST's own product rule shows they need OR-matching across L1 and L2 because the hierarchy is not clean:

> `(APM_L120 = 'Tableau Next' OR APM_L218 = 'Tableau Next')`

Note also that the board's authored labels imply a hierarchy level APM does not have. `acv-sources` names a *"Cloud & Server"* motion spanning two APM L2 values that sit beside `'Tableau Next'` as siblings; the `mix-acv` it replaced labelled Embedded as *"TabNext & CRMA"* and Agentic as *"Tab Cloud & Server"*, while APM puts Tableau Next and Tableau Server side by side as L2 siblings and "CRMA" is a name variation the docs flag for retry as "CRM Analytics".
...
### 8.2 `acv-sources` — the sales-motion split *(was `mix-acv`, Embedded / Agentic)*
