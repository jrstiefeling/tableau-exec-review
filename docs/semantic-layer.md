...
| **Per-account ACV movement** (`acv-account-fan`) | ⚠ | `Global_Combo_Name6` / `Combo_Company_Name15` (FCST) or `Account_ID106` / `Global_Combo_Name7` (SPEC). Prior year via `Close_Date_Relative_Year_clc = 'PY'` on `ACV_HISTORICALS` | FCST or SPEC — **pick one, not both** | account × relative year | dedup filter; and the utterance must request the **full result set**, not a top-N |
...
3. **A governed conformed-account identity.** `Global_Combo_Name` and `Combo_Company_Name15` are exactly the "conformed identity, not raw Org62 Account.Id" that `acv-account-fan` currently describes as an invented lineage node called "Account Hierarchy Conformance". The real thing has a name, and it is better.
...
| `acv-account-fan` | `ACV_clc` at account grain | `Sls_Forecasting_Metrics_Expanded` | account × relative year | `Org62 Opportunity` → extract → `ACV_HISTORICALS` | `accountHierarchyFieldApiName` resolves to **`Global_Combo_Name6`** or `Combo_Company_Name15` — the invented "Account Hierarchy Conformance" lineage node has a real counterpart. Note FCST has **no Account ID**, only `Account_Name129`; SPEC has `Account_ID106`. The prior-year read is `Close_Date_Relative_Year_clc = 'PY'`, not a separate point-in-time snapshot |
...
### 8.3 `acv-account-fan` — per-account movement, Q2 FY27 against Q2 FY26

The most demanding pull on the board: full population, two periods, conformed identity, and a hard reconciliation requirement.

1. Steps 1–5 of §8.1. **Additionally decide the model**, because this is the one portlet where SPEC is a live alternative — SPEC has a real `Account_ID106`, FCST has only `Account_Name129`. If account *identity* matters more than population completeness, SPEC's identifier is stronger; if the fan must reconcile to the board's FCST-sourced `ACV_clc` total, it must be FCST. **They cannot be mixed** (§1.1). Record `<TBD: model decision for the account fan>`.
2. **Resolve the conformed identity field.** `list_semantic_model_dimensions`. Confirm `Global_Combo_Name6` and `Combo_Company_Name15` (FCST) or `Global_Combo_Name7` (SPEC) exist and are visible. Determine which is the consolidated parent identity — the labels say "Global combo/consolidated account name" and "Combo company name", and the distinction between them is `<TBD: confirm which is the parent identity via get_semantic_model_logical_view or a distinct-values probe>`. **This, not `Account_Name`, is the field that prevents a re-parented subsidiary reading as one full non-renewal beside one phantom expansion.**
3. **Pull both years in one grouped query, using relative year.** The prior-year read is *not* a separate point-in-time snapshot; it is the `'PY'` rows of the same measure:
   ```
   utterance: "Return ACV (ACV_clc) grouped by Global_Combo_Name6 and by
   ...
   ```
   The "do not limit" phrasing is required (§2.6). Do **not** add `Is_Current_Quarter_clc` — that flag is for Y/Y *measures*, and on a grouped two-year pull it double-counts (§2.5).
4. **Re-sort client-side.** SPEC is explicit that server-side ordering is not to be relied on.
5. **Run the four checks the portlet already specifies**, against live results:
   - **Roll-up:** the sum of the per-account `'CY'` column must equal the certified quarter total from §8.1. If it does not, the fan is a second competing number and must not render.
   - **Conformed identity:** confirm the key is the combo/consolidated name, not a raw account name.
   - **Point-in-time:** confirm the prior year came from `'PY'` rows of the same measure, not from current account state filtered to last year's dates.
   - **One formula:** both years must be `ACV_clc`. They are, by construction, if step 3 was a single grouped query — which is the argument for doing it that way rather than as two pulls.
6. **New logos.** Accounts with no `'PY'` row have an undefined movement index and must be returned, summed, and rendered as an inflow stub outside the indexed axis. Cross-check that sum against `NNAOV_Commit_clc` **only after** reading §4.1 — that measure is a commit, so the two are not expected to agree, and a mismatch here is information rather than a bug.
7. **Row volume.** The mock population is 278 rows. A live pull at combo-account grain for one scope and two years should be the same order of magnitude. If the response is truncated, **fail loudly** — a silently truncated fan still draws, and still looks right.
...
Still owed, and **not** addressed by the owner's answer: neither model has any counterpart to the board's `"growth"` tier — the board uses `enterprise` / `commercial` / `growth` elsewhere, which matches neither vocabulary, and `acv-account-fan`'s segment thresholds should be relabelled to one of the two real value sets. Also still open: **`ESMB`**, which `Segment10` carries and the board's four columns do not show.
